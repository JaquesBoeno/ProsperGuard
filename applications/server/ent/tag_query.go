...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withTransactions; query != nil {
		if err := tq.loadTransactions(ctx, query, nodes,
...
func (tq *TagQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Transaction)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(transaction.FieldID), joinT.C(tag.TransactionsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.TransactionsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.TransactionsPrimaryKey[1]))
